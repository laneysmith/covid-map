...
export default function useFetchDataReducer(): any {
  ...
  const dispatchFetchDataSuccess = (payload: ResponseData): void =>
    dispatch({ event: EventType.FETCH_DATA_SUCCESS, payload });
  const dispatchFetchDataFailure = (error: object): void =>
    dispatch({ event: EventType.FETCH_DATA_FAILURE, error });
